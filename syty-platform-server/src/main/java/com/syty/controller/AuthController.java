...
@Tag(name = "平台认证管理", description = "平台管理员登录、登出")
public class AuthController {
    ...
    public ApiResult<LoginResponse> login(@Valid @RequestBody LoginRequest request) {
        // 1. 平台端强制查找超管 (tenant_id IS NULL)
        SysUser user = sysUserMapper.selectSuperAdmin(request.getUsername());
        
        if (user == null) {
            log.warn("平台登录失败: 找不到超管账号, username={}", request.getUsername());
            throw new BizException(400, "账号不存在或无平台权限");
        }

        // 2. 校验密码
        if (!passwordEncoder.matches(request.getPassword(), user.getPassword())) {
            log.warn("平台登录失败: 密码错误, userId={}", user.getId());
            throw new BizException(400, "密码错误");
        }
        
        // 3. 校验用户状态
        if (user.getStatus() != null && user.getStatus() == 0) {
            log.warn("平台登录失败: 用户已停用, userId={}", user.getId());
            throw new BizException(403, "账号已被停用");
        }

        // 4. 校验角色：必须包含 SUPER_ADMIN
        List<String> checkRoles = sysRoleMapper.selectRoleCodesByUserIdIgnoreTenant(user.getId());
        boolean isSuperAdmin = checkRoles != null && checkRoles.contains("SUPER_ADMIN");
        if (!isSuperAdmin) {
            log.warn("平台登录失败: 非超管尝试登录, userId={}, roles={}", user.getId(), checkRoles);
            throw new BizException(403, "非平台管理员禁止登录平台端");
        }
