<template>
  <div class="admin-performance-page">
    <div class="page-header">
      <h2 class="page-title">穿线师绩效看板（管理端）</h2>
      <!-- 门店筛选 -->
      <a-select
...
const shopList = ref<AdminShopInfo[]>([])
const shopLoading = ref(false)

const stats = reactive<AdminPerformanceStats>({
  totalOrders: 0,
  totalStringingFee: 0,
  avgDuration: 0,
...
async function fetchStats() {
  ...
  try {
    const res: any = await getAdminPerformanceStats({
      period: 'month',
...
// ==================== 生命周期 ====================
onMounted(async () => {
  await Promise.all([fetchShops(), fetchStats(), fetchRank()])
