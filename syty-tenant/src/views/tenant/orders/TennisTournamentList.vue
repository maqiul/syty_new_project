    ...
    <a-card>
      <template #title>
        <div style="display: flex; justify-content: space-between">
          <span>🎾 网球大赛记录</span>
          <a-button type="primary" @click="$router.push('/tennis/tournament')">新建登记</a-button>
        </div>
      </template>
    ...
    <a-modal v-model:open="detailVisible" title="比赛详情" width="500px" @cancel="detailVisible = false">
      <a-descriptions v-if="detail" :column="2" bordered size="small">
        ...
        <a-descriptions-item label="穿线需求" :span="2" v-if="detail.racketBrand || detail.stringType">
          {{ [detail.racketBrand, detail.racketModel, detail.stringType, (detail.tension || detail.mainTension) + 'lbs'].filter(Boolean).join(' | ') || '-' }}
        </a-descriptions-item>
      </a-descriptions>
    </a-modal>
  </div>
...
import { getTennisTournamentPage, completeTennisTournamentOrder, deleteTennisTournamentOrder } from '@/api'
...
const loadData = async () => {
  loading.value = true
  try { const r: any = await getTennisTournamentPage({ page: pagination.current, size: pagination.pageSize }); tableData.value = (r.data as any)?.records || []; pagination.total = (r.data as any)?.total || 0 }
...
const completeOrder = async (id: number) => { await completeTennisTournamentOrder(id); loadData(); message.success('已完成') }
const handleDelete = (id: number) => Modal.confirm({ title: '确认？', onOk: async () => { await deleteTennisTournamentOrder(id); loadData(); message.success('已删除') } })
onMounted(loadData)
</script>
