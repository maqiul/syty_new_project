  ...
  <div style="max-width: 800px; margin: 0 auto">
    <a-card title="🎾 网球大赛登记">
      <a-form :model="form" :label-col="{ span: 4 }" :wrapper-col="{ span: 16 }" ref="formRef">
        ...
        <a-form-item label="比赛名称">
          <a-input v-model:value="form.matchName" placeholder="如 2026 Wimbledon" />
        </a-form-item>
        ...
        <a-row :gutter="16">
          ...
          <a-col :span="12">
            <a-form-item label="比分" :label-col="{ span: 8 }">
              <a-input v-model:value="form.matchResult" placeholder="如 6:4,3:6,6:3" />
            </a-form-item>
          </a-col>
        </a-row>
    ...
    <a-card v-if="createdOrder" title="✅ 登记成功" style="margin-top: 20px; text-align: center">
      ...
      <a-space style="margin-top: 16px">
        ...
        <a-button type="primary" @click="router.push('/tennis/tournament/list')">查看记录</a-button>
      </a-space>
    </a-card>
  </div>
</template>
...
import { getShopList, createTennisTournamentOrder } from '@/api'
...
const handleSubmit = async () => {
  ...
  try {
    const res: any = await createTennisTournamentOrder({
      shopId: form.shopId,
