<template>
  <div class="tennis-register-page">
    <div class="container">
      <div class="form-card">
        <!-- 头部 -->
        <div class="card-header tennis-header">
          ...
          <a-form :model="formData" layout="vertical" @finish="submitForm">
            ...
            <div class="section">
              ...
              <a-form-item label="拍子品牌及型号" name="racketModel" :rules="[{ required: true, message: '请输入拍子型号' }]">
                <a-input v-model:value="formData.racketModel" placeholder="例如：Wilson Pro Staff 97" size="large" />
              </a-form-item>
            ...
            <div class="submit-section">
              <a-button type="primary" html-type="submit" size="large" :loading="loading" block class="tennis-btn">
...
const fetchStrings = async () => {
  try {
    const res = await axios.get('/api/tennis/string-info/list')
    stringOptions.value = res.data.data || []
  } catch (e) {
    console.error('获取网球线列表失败', e)
    // 如果接口不存在，使用通用球线
    try {
      const res = await axios.get('/api/string-info/list')
      stringOptions.value = res.data.data || []
    } catch (e2) {
      console.error('获取球线列表失败', e2)
    }
  }
}
...
const submitForm = async () => {
  ...
  try {
    const payload = {
      ...
      source: 'customer_register_tennis'
    }

    const res = await axios.post('/api/tennis/tournament/customer', payload)
