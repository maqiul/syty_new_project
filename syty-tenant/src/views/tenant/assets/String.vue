  ...
  <div>
    ...
    <a-modal v-model:open="dialogVisible" :title="dialogForm.id ? '编辑球线' : '新增球线'" width="500px" @ok="handleSave" :confirmLoading="saving">
      <a-form :model="dialogForm" :label-col="{ span: 6 }" :wrapper-col="{ span: 16 }">
        ...
        <a-form-item label="型号" required><a-input v-model:value="dialogForm.model" placeholder="BG65 / BG80..." /></a-form-item>
...
import { getStringPage, addString, updateString, deleteString } from '@/api'
...
const loadData = async () => { loading.value = true; const r: any = await getStringPage({ page: pagination.current, size: pagination.pageSize }); tableData.value = r.data.records || r.data.list || []; pagination.total = r.data.total || 0; loading.value = false }
...
const handleSave = async () => { saving.value = true; try { if (dialogForm.id) await updateString(dialogForm); else await addString(dialogForm); message.success('ok'); dialogVisible.value = false; loadData() } catch { message.error('fail') } finally { saving.value = false } }
const handleDelete = (id: number) => Modal.confirm({ title: '确认删除？', onOk: async () => { await deleteString(id); loadData(); message.success('已删除') } })
onMounted(loadData)
</script>
