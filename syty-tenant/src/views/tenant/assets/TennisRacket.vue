  ...
  <div>
    ...
    <a-modal v-model:open="dialogVisible" :title="dialogForm.id ? '编辑球拍' : '新增球拍'" width="500px" @ok="handleSave" :confirmLoading="saving">
      <a-form :model="dialogForm" :label-col="{ span: 6 }" :wrapper-col="{ span: 16 }">
        <a-form-item label="品牌" required><a-input v-model:value="dialogForm.brand" placeholder="Wilson / Babolat / Head..." /></a-form-item>
        <a-form-item label="型号" required><a-input v-model:value="dialogForm.model" placeholder="Pro Staff 97 / Pure Drive..." /></a-form-item>
...
import { getTennisRacketPage, addTennisRacket, updateTennisRacket, deleteTennisRacket } from '@/api'
...
const loadData = async () => {
  loading.value = true
  const res: any = await getTennisRacketPage({ page: pagination.current, size: pagination.pageSize })
...
const handleSave = async () => { saving.value = true; try { if (dialogForm.id) await updateTennisRacket(dialogForm); else await addTennisRacket(dialogForm); message.success('ok'); dialogVisible.value = false; loadData() } catch { message.error('fail') } finally { saving.value = false } }
const handleDelete = (id: number) => { Modal.confirm({ title: '确认删除？', onOk: async () => { await deleteTennisRacket(id); loadData(); message.success('已删除') } }) }
onMounted(loadData)
</script>
