...
import { getPlayerPage, addPlayer, updatePlayer, deletePlayer, uploadFile } from '@/api'
...
const loadData = async () => {
  loading.value = true
  const res: any = await getPlayerPage({ page: pagination.current, size: pagination.pageSize })
...
const handleSave = async () => {
  saving.value = true
  try {
    if (dialogForm.id) await updatePlayer(dialogForm)
    else await addPlayer(dialogForm)
...
const handleDelete = (id: number) => {
  Modal.confirm({ title: '确认删除？', onOk: async () => { await deletePlayer(id); loadData(); message.success('已删除') } })
}
